...
import { uid, fmt, today, nombreMes } from '../lib/helpers'
...
const Comisiones = ({ conductores, viajes, pagos, camiones, pagosComision = [], setPagosComision }) => {
  ...
  const save_ = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('pagos_comision').insert([{ ...rest, comprobante_url, user_id: user.id }]).select()
  ...
  const del = async p => {
    ...
    const { error } = await supabase.from('pagos_comision').delete().eq('id', p.id)
          ...
          <div key={conductor.id} style={{ background:C.bg1, border:`1px solid ${C.border}`, borderRadius:"9px", overflow:"hidden" }}>
            <div style={{ padding:"10px 13px", background:C.bg2, borderBottom:`1px solid ${C.border}` }}>
              <div style={{ display:"flex", alignItems:"center", gap:"6px", marginBottom:"8px" }}>
                ...
                <Badge label={`${conductor.comision || 0}%`} color="blue"/>
            ...
            {detalle.map(({ viaje, cam, generado:gen, porPagar:pp, saldado }) => (
              <div key={viaje.id} style={{ padding:"9px 13px", borderBottom:`1px solid ${C.bg3}`, display:"flex", justifyContent:"space-between", alignItems:"center", gap:"8px" }}>
                <div style={{ minWidth:0, flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"6px", flexWrap:"wrap" }}>
                    <span style={{ fontSize:"11px", fontWeight:700, color:C.accentLight }}>{viaje.numero}</span>
                    <span style={{ fontSize:"10px", color:C.textMuted }}>{viaje.origen} → {viaje.destino}</span>
                    <Badge label={saldado ? "Cobrado" : "Por cobrar"} color={saldado ? "green" : "yellow"}/>
                  </div>
                  <div style={{ fontSize:"10px", color:C.textMuted, marginTop:"1px" }}>
                    {viaje.salida}{cam ? ` · ${cam.placa}` : ""} · Flete ${fmt(viaje.flete)}
                  </div>
                </div>
